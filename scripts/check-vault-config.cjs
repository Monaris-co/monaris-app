...
async function main() {
  ...
  // Load contract addresses from contracts.json or .env
  const contractsPath = path.join(__dirname, "..", "contracts.json");
  let vaultAddress, usmtPlusAddress, demoUSDCAddress;

  if (fs.existsSync(contractsPath)) {
    const contractsData = JSON.parse(fs.readFileSync(contractsPath, "utf8"));
    vaultAddress = contractsData.contracts?.Vault;
    usmtPlusAddress = contractsData.contracts?.USMTPlus;
    demoUSDCAddress = contractsData.contracts?.DemoUSDC;
  }

  // Fallback to environment variables
  vaultAddress = vaultAddress || process.env.VITE_VAULT_ADDRESS;
  usmtPlusAddress = usmtPlusAddress || process.env.VITE_USMT_PLUS_ADDRESS;
  demoUSDCAddress = demoUSDCAddress || process.env.VITE_DEMO_USDC_ADDRESS;
  ...
  try {
    ...
    console.log("Deployer allowance for Vault:", hre.ethers.formatUnits(allowance, 6), "USDC");
    if (allowance === 0n) {
  ...
  try {
    ...
    console.log("Total Borrowed:", hre.ethers.formatUnits(totalBorrowed, 6), "USDC");
  } catch (error) {
