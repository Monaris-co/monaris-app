...
async function main() {
  ...
  // Contract addresses from .env / deployment
  ...
  console.log("\n3. Testing createInvoice (dry run with callStatic)...");
  try {
    // Test parameters
    ...
    const testMetadataHash = ethers.ZeroHash;
  ...
  console.log("\n=== Summary ===");
  console.log("If any checks failed, run the fix script to grant roles/set addresses.");
}
